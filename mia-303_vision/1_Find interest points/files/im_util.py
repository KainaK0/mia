...
def convolve_rows(im, k):
  ...
  *****************************************
  ...
  The output should be the same size as the input
  You can assume zero padding, and an odd-sized kernel
  Note: Your implementation should work for an RGB image.
  Hint: Use the convolve_1d you just implemented!
  """
...
def gauss_kernel(sigma):
  ...
  *******************************************
  Hint: Can you use numpy to efficiently compute the gaussian kernel without using for loops?
  """
...
def convolve_gaussian(im, sigma):
  ...
  ***************************************
  Hint: Use the gaussian kernel you just implemented!
  """
...
def compute_gradients(img):
  ...
  ***********************************************
  Hint: Smooth the image first to avoid noisy output and then use appropriate odd length kernels to find the gradients 
  """
...
def convolve(im, kernel):
  ...
  kh, kw = kernel.shape
  kh1, kw1 = (kh//2, kw//2)
  kh2, kw2 = (kh-1-kh1, kw-1-kw1)
  imc = np.zeros((H, W, B))
  for band in range(B):
    i=np.pad(im[:, :, band],((kh1,kh2),(kw1,kw2)),mode='edge')
    imc[:, :, band] = sps.correlate2d(i, kernel, mode='valid')
  return imc
